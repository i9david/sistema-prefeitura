...
import {
  atualizarVinculoProfessorModalidade,
  excluirVinculoProfessorModalidade,
  vincularProfessorNaModalidade,
} from './actions'
...
export default async function ModalidadeProfessoresPage({
  searchParams,
}: {
  ...
  if (erroModalidades) {
    redirect(`/modalidade-professores?message=${encodeURIComponent(erroModalidades.message)}`)
  }

  if (erroProfessores) {
    redirect(`/modalidade-professores?message=${encodeURIComponent(erroProfessores.message)}`)
  }
  ...
  return (
    <main className="min-h-screen p-6 bg-slate-50">
      <div className="mx-auto grid max-w-7xl gap-6 lg:grid-cols-[300px_1fr]">
        <ModuloCentroCulturalNav currentPath="/modalidade-professores" />
        ...
        <section className="space-y-6">
            ...
            <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
              <div>
                ...
                <p className="mt-2 text-sm text-slate-600">
                  Vincule professores às modalidades e defina a função de cada um
                </p>
              </div>
          ...
          {mostrarFormulario && (
            <div className={cardClassName()}>
              ...
              <form
                action={vinculoEditando ? atualizarVinculoProfessorModalidade : vincularProfessorNaModalidade}
          ...
          <div className={cardClassName()}>
            <div>
              ...
              <p className="mt-1 text-sm text-slate-600">
                Veja quais professores estão vinculados a cada modalidade
              </p>
            </div>
                      ...
                      <tr key={vinculo.id} className="border-b">
                        ...
                        <td className="px-4 py-3">
                          <div className="flex flex-col gap-2">
                            <a
                              href={`/modalidade-professores?editar=${vinculo.id}`}
                              className="rounded-lg border border-slate-300 px-3 py-2 text-center text-slate-700"
                            >
                              Editar
                            </a>

                            <form action={excluirVinculoProfessorModalidade}>
